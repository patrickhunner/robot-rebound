...
On free Render deployments, the Discord Activity may take up to two minutes to wake the server after inactivity. The preparation screen retries the health check automatically before requesting Discord authorization, provides a **Wake server now** button for an immediate extra ping, and offers an in-app Retry button if the server is still unavailable.
