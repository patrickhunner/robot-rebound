...
4. The first one-time bid starts the configured bidding timer. Players type a count and press Enter or use a 1–30 quick bid. Every new bid is announced to all current players in a short queued banner and screen-edge flash without blocking the board. If every connected eligible player submits before expiry, proof begins immediately.
...
Robot position changes slide smoothly across the board at a constant cells-per-second rate, so longer slides take proportionally longer. Automatic optimal-strategy playback waits for each slide to finish, followed by a 75 ms safety gap, before beginning the next move. The host can change the shared speed from 1 (slowest) through 10 (fastest) at any time; new rooms begin at speed 5.
