...
8. A success awards a point and opens a shared review from the revealed starting positions. Everyone may demonstrate alternatives with clicks or arrow keys, reset the shared board, and hold one robot lock at a time. A background solver displays up to five shortest strategies, each of which can be played for the whole room. Automatic playback resets the board and temporarily blocks manual review interaction. These demonstrations do not change scores.
...
Robot position changes slide smoothly across the board. The host can change the shared speed from 1 (slowest) through 10 (fastest) at any time; new rooms begin at speed 5.
